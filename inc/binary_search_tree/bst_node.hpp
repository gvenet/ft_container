...
struct bst_node {
		...
		bst_node()
				: parent(), left(), right(), succ(), pred(), value(), is_limit( false ), left_depth(),
					right_depth() { }
		...
		bst_node( const value_type& val, bool b = false )
				: parent(), left(), right(), succ(), pred(), value( val ), is_limit( b ), left_depth(),
					right_depth() { }

		bst_node( const pointer& cpy )
				: parent(), left( cpy->left ), right( cpy->right ), succ( cpy->succ ), pred( cpy->pred ),
					value( cpy->value ), is_limit( cpy->is_limit ), left_depth( cpy->left_depth ),
